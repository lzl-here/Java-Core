...
public class Threads {
    ...
    private static synchronized void initialize(TypeDataBase db) {
        ...
        boolean foundCRSCommandListenerThread = false;
        try{
            db.lookupType("CRSCommandListenerThread");
            foundCRSCommandListenerThread = true;
        } catch ( RuntimeException re ) {}
        if (foundCRSCommandListenerThread) {
            virtualConstructor.addMapping("CRSCommandListenerThread", CRSCommandListenerThread.class);
        }

        boolean foundCRSAgentInitThread = false;
        try{
            db.lookupType("CRSAgentInitThread");
            foundCRSAgentInitThread = true;
        } catch ( RuntimeException re ) {}
        if (foundCRSAgentInitThread) {
            virtualConstructor.addMapping("CRSAgentInitThread", CRSAgentInitThread.class);
        }
    }
