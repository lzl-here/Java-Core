...
public class ReentrantLock implements Lock, java.io.Serializable {
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        @ReservedStackAccess
        final void lock() {
            // 线程进来先尝试CAS AQS的state，修改成功就说明之前AQS处于无锁状态，假如是重入情况就state++，此时该线程抢锁成功
            // 公平 和 非公平 情况下，tryLock逻辑不一样，公平模式需要判断线程等待队列为空才抢锁（也就是没有竞争），不为空就入队，非公平不判断是否为空，直接CAS尝试抢锁
            if (!initialTryLock()) {
                // 失败就进入acquire，准备入队列（acquire里面还会进行一次抢锁）
                acquire(1);
            }
        }
