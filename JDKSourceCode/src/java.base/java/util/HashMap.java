...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    // @description: capacity * loadFactor，size大于这个值进行扩容
    int threshold;
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        ...
        // 如果slot是空的
        if ((slotHead = tab[i = (n - 1) & hash]) == null) {
            tab[i] = newNode(hash, key, value, null);
        }
        // 如果slot不是空的，拉链法挂载
        else {
            Node<K, V> slotNode = slotHead;
            Node<K, V> next;
            K k;

            // 下面就是遍历这个slot查找key的过程了

            // 看slot的头节点是不是这个key
            if (slotNode.hash == hash && ((k = slotNode.key) == key || (key != null && key.equals(k)))) {
                next = slotNode;
            }
            //
            else if (slotNode instanceof TreeNode) {
                next = ((TreeNode<K, V>) slotNode).putTreeVal(this, tab, hash, key, value);
            }
            // 遍历链表查找
            // 假如链表中没有，就挂载到末尾
            // 假如链表中有，就根据evict看是否需要更新
            // 期间假如发现链表长度>=7，会进行树化
            else {
                for (int binCount = 0; ; ++binCount) {
                    // 找到底了也没有，说明slot下没有这个key，挂载到下面
                    if ((next = slotNode.next) == null) {
                        slotNode.next = newNode(hash, key, value, null);
                        // 假如发现链表长度>=7，进行树化
            ...
            // 上面一大段都是在找这个slot下有没有这个key，假如找到了
